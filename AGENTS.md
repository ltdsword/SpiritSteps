...
- **Verified Unity 6 enum mapping:** `m_ScaleMode: 0` is `ConstantPixelSize`, `1` is `ConstantPhysicalSize`, and `2` is `ScaleWithScreenSize` (confirmed directly in Unity 6000.3.16f1 via `PanelScaleMode`). The polished UI is authored on a **720×1600 logical canvas** and validated at 1080×2400, where it renders at exactly 1.5×. Both Home and AR panel assets therefore use `m_ScaleMode: 2`, reference resolution 720×1600, and Match Width Or Height at 0.5. `ARWalkingAppUISettings.m_AutoCorrectUiScale` is disabled so App UI cannot replace that explicit contract with DPI-based physical scaling. If a future screen looks unexpectedly tiny or huge, verify these values before changing individual font sizes.
